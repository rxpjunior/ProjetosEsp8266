...
// WiFi
#define NET_SSID "ETEC-IoT2" // Substituir SSID pelo nome da rede WiFi
#define NET_PASSWORD "etecjb*123"  // Substituir SENHA pela senha da rede WiFi
 

//Configuração do Broker MQTT
#define MQTT_ID "KaJoRoSA" //Substituir MQTT-ID pela identificação do cliente no MQTT no Broker
...
#define MQTT_TOPIC "ETEC-IoT" //Substituir MQTT-TOPIC pelo tópico a ser utiizado no MQTT Broker
...
void loop(void) {
  ...
  MQTT.publish(MQTT_TOPIC, newHostname.c_str());
  MQTT.publish(MQTT_TOPIC, String(millis()).c_str());
